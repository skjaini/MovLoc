...
angular.module('film.controllers', []).
  controller('MapCtrl', ['$scope', '$http', 'initService', function($scope, $http, initService) {
    ...
    // Fetch the list of movie names
    $http.get('js/movie-names.json')
	.then(function(res){
	    $scope.movies = res.data.movieNames;
	});
    ...
    // Initialize Parse to query moviedata class
    Parse.initialize("VrmI0v762cXbSobJ49uKxDBNH33bcAXK5QDixZuv", "WY7mBM1jLW4MlrWr0zkr4wusS2Z6mCPQGA3IwTzw");
    var moviedata = Parse.Object.extend("moviedata");
    var query = new Parse.Query(moviedata);
    ...
    // Find the location given a movie name
	...
	// Filter for the selected movieName
	query.equalTo("title", $scope.movieName);

	// Fetch movie locations
	query.find({
		...
		if (queryResults.length > 0) {
		    ...
		    // Fetch the lat/long for each location 
		    for (var i = 0; i < queryResults.length; i++) {
			var object = queryResults[i];

			var streetAddress = object.get('locations');
